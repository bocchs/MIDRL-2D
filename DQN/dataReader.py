...
def getLandmarksFromTXTFile(file):
    ...
    with open(file) as fp:
        ...
        landmarks = np.flip(landmarks) # reverse landmark coordinates
        return landmarks
...
class filesListBrainMRLandmark(object):
    ...
    def __init__(self, files_list=None, returnLandmarks=True):
        ...
        try:
            ...
            self.image_files = files_list # only one file from command line
            self.cmdline = True
    ...
    def sample_circular(self,shuffle=False):
        ...
        if self.cmdline:
            while True:
                    sitk_image, image = NiftiImage().decode(self.image_files)
                    if self.returnLandmarks:
                        assert False
                    else:
                        landmark = None
                    # extract filename from path
                    idx = self.image_files.rfind('/')
                    image_filename = self.image_files[idx+1:-7]
            ...
            while True:
                for idx in indexes:
                    ...
                    if self.returnLandmarks:
                        ...
                        landmark = all_landmarks[0] # set desired landmark index ###################################################################
                        ...
                        landmark = None
                    # extract filename from path
                    image_filename = self.image_files[idx].split('.nii')[0]
                    yield image, landmark, image_filename, sitk_image.GetSpacing()
...
class NiftiImage(object):
    ...
    def decode(self, filename,label=False):
        ...
        image.data = sitk.GetArrayFromImage(sitk_image).transpose(1,0)#.astype('uint8')
        # image.data = sitk.GetArrayFromImage(sitk_image)
