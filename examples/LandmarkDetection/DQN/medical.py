...
class MedicalPlayer(gym.Env):
    ...
    def __init__(self, directory=None, viz=False, task=False, files_list=None,
        ...
        self.target_rectangle = Rectangle(0, 0, 0, 0)
    ...
    def new_random_game(self):
        ...
        #print(self.filename)
        #print(self.filepath)
        if "heart" in self.filepath:
            self.landmark = "heart"
        elif "kidney" in self.filepath:
            self.landmark = "kidney"
        elif "trochanter" in self.filepath:
            self.landmark = "trochanter"
        elif "knee" in self.filepath:
            self.landmark = "knee"
        elif "prostate" in self.filepath:
            self.landmark = "prostate"
        elif "breast" in self.filepath:
            self.landmark = "breast"
    ...
    def step(self, act, qvalues):
        ...
        distance_error_pixels = self.cur_dist
        curr_space = 1.5625 # for Dixon and T1
        if "_t2_" in self.filepath:
            curr_space = 1.9531
        # view notes.txt for prostate patient spacing

        distance_error_mm = self.calcDistance(self._location,
                                                  self._target_loc,
        ...
        info = {'score': self.current_episode_score.sum, 'gameOver': self.terminal,
                'distErrorPixels': distance_error_pixels, 'distErrorMM': distance_error_mm, 'filename': self.filename}

        # print(self.reward)
        intersection = area(self.rectangle, self.target_rectangle)
        area_rectangle = area(self.rectangle, self.rectangle)
        area_target_rectangle = area(self.target_rectangle, self.target_rectangle)
        union = area_rectangle + area_target_rectangle - intersection
        self.dice = 2 * intersection / (area_rectangle + area_target_rectangle)
        self.overlap = intersection / union
    ...
    def _oscillate(self):
        ...
        # freq[i] is ((x,y,z), count)
        # freq[0][0] is (x,y,z) with highest count
        # freq[0][1] is the highest count
        if freq[0][0] == (0, 0):
            if (freq[1][1] > 3):
                return True
            else:
    ...
    def display(self, return_rgb_array=False):
        ...
        if self.saveGif and self.terminal:
            ...
            pngname = self.filename.split('.')[0] + '.png'
            im.save(pngname)
            # sys.exit()
        """
        ...
        if (self.task != 'play'):
            ...
            self.path.append(current_point) # record path
            #self.viewer.draw_circle(radius=1, pos_x=current_point[0], pos_y=current_point[1], color=(0,204,0,255))
            self.viewer.draw_circle(radius = scale_x * 1,
                                    pos_x = scale_x*target_point[0],
                                    pos_y = scale_y*target_point[1],
            ...
            if self.xscale % 2:
                targ_xmin = target_point[0] - int(self.width * self.xscale / 2) - 1
                targ_xmax = target_point[0] + int(self.width * self.xscale / 2)
                targ_ymin = target_point[1] - int(self.height * self.yscale / 2) - 1
                targ_ymax = target_point[1] + int(self.height * self.yscale / 2)
            else:
                targ_xmin = target_point[0] - round(self.width * self.xscale / 2)
                targ_xmax = target_point[0] + round(self.width * self.xscale / 2)
                targ_ymin = target_point[1] - round(self.height * self.yscale / 2)
                targ_ymax = target_point[1] + round(self.height * self.yscale / 2)

            self.target_rectangle = Rectangle(targ_xmin,targ_xmax,targ_ymin,targ_ymax)
            self.viewer.draw_rect(scale_x*self.target_rectangle.xmin, scale_y*self.target_rectangle.ymin,
                                  scale_x*self.target_rectangle.xmax, scale_y*self.target_rectangle.ymax, target=True)
            # self.viewer.display_text('Ground Truth ', color=(255, 0, 0, 255),
            #                          x=self.target_rectangle.xmax,
            #                          y=self.target_rectangle.ymax-50)
            # display info
            # color = (0,204,0,255) if self.reward>0 else (204,0,0,255)
            # text = 'Error ' + str(round(self.cur_dist,3)) + 'mm'
            # text = 'Error ' + str(round(self.cur_dist,3)) + ' pixels'
        ...
        if self.saveGif and self.terminal:
            ...
            if self.terminal:
                gifname = self.filename.split('.')[0] + '_terminal.gif'
                # save combined agent and ground truth
                ...
                arr = np.flip(np.reshape(arr, (image_data.height, image_data.width, -1)), 0)
                im = Image.fromarray(arr)
                pngname = self.filename.split('.')[0] + '_terminal_target.png'
                im.save(pngname)
                ...
                arr = np.flip(np.reshape(arr, (image_data.height, image_data.width, -1)), 0)
                im = Image.fromarray(arr)
                pngname = self.filename.split('.')[0] + '_terminal_agent.png'
                im.save(pngname)

                #sys.exit()
                #self.viewer.saveGif(gifname, arr=self.terminal_img, duration=self.viz)
